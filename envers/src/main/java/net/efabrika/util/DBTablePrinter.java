...
/**
 * Just a utility to print rows from a given DB table or a <code>ResultSet</code> to
 * standard out, formatted to look like a table with rows and columns with borders.
 *
 * <p>
 * Stack Overflow website
 * (<a target="_blank" href="http://stackoverflow.com">stackoverflow.com</a>) was the
 * primary source of inspiration and help to put this code together. Especially the
 * questions and answers of the following people were very useful:
 * </p>
 *
 * <p>
 * Question: <a target="_blank" href="http://tinyurl.com/q7lbqeh">How to display or print
 * the contents of a database table as is</a><br>
 * People: sky scraper
 * </p>
 *
 * <p>
 * Question: <a target="_blank" href="http://tinyurl.com/pbwgess">System.out.println()
 * from database into a table</a><br>
 * People: Simon Cottrill, Tony Toews, Costis Aivali, Riggy, corsiKa
 * </p>
 *
 * <p>
 * Question: <a target="_blank" href="http://tinyurl.com/7x9qtyg">Simple way to repeat a
 * string in java</a><br>
...
public class DBTablePrinter {
  ...
  /**
   * Column type category for <code>CHAR</code>, <code>VARCHAR</code> and similar text
   * columns.
   */
  ...
  /**
   * Column type category for date and time related columns like <code>DATE</code>,
   * <code>TIME</code>, <code>TIMESTAMP</code> etc.
   */
  ...
  /**
   * Column type category for types for which the type name will be printed instead of the
   * content, like <code>BLOB</code>, <code>BINARY</code>, <code>ARRAY</code> etc.
   */
  ...
  private static class Column {
     ...
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.Types
     * </a>.
     */
    ...
    /**
     * Width of the column that will be adjusted according to column label and values to
     * be printed.
     */
    private int width = 0;
    ...
    /**
     * Column type category. The columns will be categorised according to their column
     * types and specific needs to print them correctly.
     */
    private int typeCategory = 0;

    /**
     * Constructs a new <code>Column</code> with a column label, generic SQL type and type
     * name (as defined in <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.Types
     * </a>)
     *
    ...
    /**
     * Adds a <code>String</code> representation (<code>value</code>) of a value to this
     * column object's {@link #values} list. These values will come from each row of a
     * <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html"> ResultSet </a>
     * of a database query.
     *
    ...
    /**
     * Returns the column value at row index <code>i</code>. Note that the index starts at
     * 0 so that <code>getValue(0)</code> will get the value for this column from the
     * first row of a <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html"> ResultSet</a>.
     *
    ...
    /**
     * Returns the value of the {@link #justifyFlag}. The column values will be printed
     * using <code>String.format</code> and this flag will be used to right or left
     * justify the text.
     *
    ...
    /**
     * Sets {@link #justifyFlag} to <code>"-"</code> so that the column value will be left
     * justified when printed with <code>String.format</code>. Typically numbers will be
     * right justified and text will be left justified.
     */
  ...
  /**
   * Overloaded method that prints rows from table <code>tableName</code> to standard out
   * using the given database connection <code>conn</code>. Total number of rows will be
   * limited to {@link #DEFAULT_MAX_ROWS} and {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be
   * used to limit the width of text columns (like a <code>VARCHAR</code> column).
   *
   ...
   * Overloaded method that prints rows from table <code>tableName</code> to standard out
   * using the given database connection <code>conn</code>. Total number of rows will be
   * limited to <code>maxRows</code> and {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used
   * to limit the width of text columns (like a <code>VARCHAR</code> column).
   *
   * @param conn Database connection object (java.sql.Connection)
   * @param tableName Name of the database table
   ...
   * Overloaded method that prints rows from table <code>tableName</code> to standard out
   * using the given database connection <code>conn</code>. Total number of rows will be
   * limited to <code>maxRows</code> and <code>maxStringColWidth</code> will be used to
   * limit the width of text columns (like a <code>VARCHAR</code> column).
   *
   * @param conn Database connection object (java.sql.Connection)
   * @param tableName Name of the database table
   * @param maxRows Number of max. rows to query and print
   ...
   * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html"> ResultSet</a> to
   * standard out using {@link #DEFAULT_MAX_TEXT_COL_WIDTH} to limit the width of text
   * columns.
   *
   ...
   * Overloaded method to print rows of a <a target="_blank" href=
   * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html"> ResultSet</a> to
   * standard out using <code>maxStringColWidth</code> to limit the width of text columns.
   *
   * @param rs The <code>ResultSet</code> to print
   * @param maxStringColWidth Max. width of text columns
   */
  public static void printResultSet(ResultSet rs, int maxStringColWidth) {
    try {
      ...
      for (int i = 1; i <= columnCount; i++) {
        Column c = new Column(rsmd.getColumnLabel(i), rsmd.getColumnType(i),
       ...
       * Column objects, iterated over the ResultSet rows, populated the column values and
      ...
      StringBuilder info = new StringBuilder("Printing ").append(rowCount);
      info.append(rowCount > 1 ? " rows from " : " row from ");
      info.append(tableNames.size() > 1 ? "tables " : "table ");
      info.append(sj.toString());
  ...
  /**
   * Takes a generic SQL type and returns the category this type belongs to. Types are
   * categorized according to print formatting needs:
   * <p>
   * Integers should not be truncated so column widths should be adjusted without a column
   * width limit. Text columns should be left justified and can be truncated to a max.
   * column width etc...
   * </p>
   *
   * See also: <a target="_blank" href=
   * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.Types</a>
   *
   * @param type Generic SQL type
  ...
  private static int whichCategory(int type) {
    ...
    case Types.DATE, Types.TIME, Types.TIME_WITH_TIMEZONE, Types.TIMESTAMP, Types.TIMESTAMP_WITH_TIMEZONE -> CATEGORY_DATETIME;
    case Types.BOOLEAN -> CATEGORY_BOOLEAN;
    case Types.VARCHAR, Types.NVARCHAR, Types.LONGVARCHAR, Types.LONGNVARCHAR, Types.CHAR, Types.NCHAR -> CATEGORY_STRING;
